...
| **INV-2** | Hard spend ceiling | self-tracked cap (live: `budget.total`; post-hoc: `session.json.billable_tokens`); hook refuses past cap | seed over-cap → next fan-out denied | approximate vs provider meter; monthly USD cap not machine-readable → consent + conservative caps. **Cache-read trade-off (Issue #2):** the cap counts `billable_tokens` (in+out+cache_write, **excludes cache_read**) so a cheap, cache-read-heavy session (71.6M tokens / $61) doesn't hard-deny all fan-out — but the cap therefore tracks *cost*, not *total* token volume (what the provider rate-limit window counts). A single fan-out is still bounded by `per_fanout_cap` + the unarmed-Workflow deny (INV-1); the residual is slow window-creep under repeated cheap cache hits — accepted, since L1 (live window) guards that directly when a signal is present. |
